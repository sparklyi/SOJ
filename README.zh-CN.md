...
- API、worker 和 judge-agent 进程的 Prometheus 指标、trial alert rules，以及默认关闭的可选 OpenTelemetry tracing。
...
`make smoke-real-docker` 默认从 GHCR 拉取已发布的 runner images。如需在修改 Dockerfile 时本地构建 runner images：

```bash
RUNNER_IMAGES_PREPARE=build make smoke-real-docker
```

Runner images 由 [publish-runner-images.yml](.github/workflows/publish-runner-images.yml) 在 `main` 上 runner image 文件变更、版本 tag 和手动触发时发布。
...
Trial dashboard 查询、alert 解读，以及从 metric pivot 到 trace/attempt 的诊断流程记录在 [docs/observability-trial-loop.md](docs/observability-trial-loop.md)。默认栈不要求 Grafana、Alertmanager、Jaeger、Tempo 或 OpenTelemetry collector。
...
| `SOJ_TRACING_ENABLED` | 设置为 `true` 时启用 OpenTelemetry tracing；默认关闭。 |
...
| `OTEL_SERVICE_NAME` | 可选的 OpenTelemetry service name 覆盖值。 |
| `OTEL_RESOURCE_ATTRIBUTES` | 可选的 OpenTelemetry resource attributes，例如部署环境。 |
...
- 将 tracing backend 和 collector 保持在私有网络中；tracing 默认关闭，必须通过 `SOJ_TRACING_ENABLED=true` 显式启用。
...
- 生产环境将 `SOJ_DOCKER_RUNNER_IMAGE_GO` 和 `SOJ_DOCKER_RUNNER_IMAGE_CPP17` 固定到 release 或 `sha-*` runner image tag。
